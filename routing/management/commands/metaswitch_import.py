...
class Command(BaseCommand):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._funcs = {
            ...
            'wireless-port': self.import_wireless_port
        }
    ...
    def import_fraud_bypass(self, input_file, truncate=False):
        ...
        error_count = 0
        User = get_user_model()
        steward_user, _ = User.objects.get_or_create(username='system',
                                                     defaults={'first_name': 'System', 'last_name': ''})
                ...
                if len(row) >= 2:
                    ...
                    if trunk_number == '999999':
                        ...
                        self.stdout.write(self.style.ERROR(
                            'ERROR: Trunkgroup {} is not 999999 as required for {}'.format(trunk_number, number)))
        self.stdout.write(self.style.SUCCESS(
            'Successfully imported Fraud Bypass file >> Created: {} Updated: {} Error: {}'.format(create_count,
                                                                                                  update_count,
                                                                                                  error_count)))
            ...
            for row in csv_file:
                if len(row) == 3:
                    ...
                    if not re.match('^\d{10}$', called_number):
                        error_count += 1
                        self.stdout.write(
                            self.style.ERROR('ERROR: Called Number {} is not 10 digits'.format(called_number)))
                        continue
                    forward_number = row[2].strip()
                    if not re.match('^\d{10}$', forward_number):
                        error_count += 1
                        self.stdout.write(self.style.ERROR(
                            'ERROR: Called Number: {}, Forward Number {} is not 10 digits'.format(called_number,
                                                                                                  forward_number)))
                        continue
            ...
            for row in csv_file:
                if len(row) >= 2:
                    ...
                    if trunk_number in trunks:
                        ...
                        NumberHistory.objects.create(cc=obj.cc, number=obj.number, user=steward_user,
                                                     action='Imported to {}'.format(obj.route.name))
                        if created:
                            create_count += 1
                        else:
                            update_count += 1
                        ...
                        error_count += 1
                        self.stdout.write(
                            self.style.ERROR('ERROR: Trunkgroup {} was not found for {}'.format(trunk_number, number)))
        self.stdout.write(self.style.SUCCESS(
            'Successfully imported Route file >> Created: {} Updated: {} Error: {}'.format(create_count, update_count,
                                                                                           error_count)))
            ...
            for row in csv_file:
                if len(row) >= 3:
                    ...
                    if end_office_trunk_number in trunks and long_distance_trunk_number in trunks:
                        ...
                        obj, created = OutboundRoute.objects.update_or_create(number=destination, defaults={
                            'end_office_route': end_office_trunk, 'long_distance_route': long_distance_trunk})
                        OutboundRouteHistory.objects.create(number=destination, user=steward_user,
                                                            action='Imported to End Office "{}", Long Distance "{}"'.format(
                                                                obj.end_office_route.name,
                                                                obj.long_distance_route.name))
                        if created:
                            create_count += 1
                        else:
                            update_count += 1
                        ...
                        if end_office_trunk_number not in trunks:
                            self.stdout.write(self.style.ERROR(
                                'ERROR: End Office Trunkgroup {} was not found for {}'.format(end_office_trunk_number,
                                                                                              destination)))
                        if long_distance_trunk_number not in trunks:
                            self.stdout.write(self.style.ERROR(
                                'ERROR: Long Distance Trunkgroup {} was not found for {}'.format(
                                    long_distance_trunk_number, destination)))
        self.stdout.write(self.style.SUCCESS(
            'Successfully imported Outbound Route file >> Created: {} Updated: {} Error: {}'.format(create_count,
                                                                                                    update_count,
                                                                                                    error_count)))

    @transaction.non_atomic_requests
    def import_wireless_port(self, input_file, truncate=False):
        create_count = 0
        update_count = 0
        error_count = 0
        User = get_user_model()
        ...
        if truncate:
            WirelessPort.objects.all().delete()
            WirelessPortHistory.objects.all().delete()
        ...
        with open(input_file.name) as f:
            ...
            for row in csv_file:
                if len(row) >= 2:
                    ...
                    if trunk_number in trunks:
                        route = trunks[trunk_number]
                        obj, created = WirelessPort.objects.update_or_create(cc=1, number=number,
                                                                             defaults={'route': route})
                        WirelessPortHistory.objects.create(cc=obj.cc, number=obj.number, user=steward_user,
                                                           action='Imported to {}'.format(obj.route.name))
                        if created:
                            create_count += 1
                        else:
                        ...
                        self.stdout.write(
                            self.style.ERROR('ERROR: Trunkgroup {} was not found for {}'.format(trunk_number, number)))
        self.stdout.write(self.style.SUCCESS(
            'Successfully imported Wireless Port file >> Created: {} Updated: {} Error: {}'.format(create_count,
                                                                                                   update_count,
                                                                                                   error_count)))
